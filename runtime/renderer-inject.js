((cssText, artDataUrl, themeConfig) => {
  ...
  const ART_ATTRS = [
    ...
    "data-dream-art-ready", "data-dream-theme-variant",
  ];
  const VERSION = __DREAM_SKIN_VERSION_JSON__;
  const STYLE_REVISION = __DREAM_SKIN_STYLE_REVISION_JSON__;
  const THEME = themeConfig && typeof themeConfig === "object" ? themeConfig : {};
  const THEME_VARIANT = /(?:RX-?78|WHITE BASE)/i.test(`${THEME.name || ""} ${THEME.quote || ""}`)
    ? "rx78-white-base" : "default";
  ...
  const applyRootState = (root) => {
    ...
    setAttribute(root, "data-dream-theme-variant", THEME_VARIANT);
    setStyleProperty(root, "--dream-skin-art", `url("${artUrl}")`);
    /* 路由标记（Yihan 2026-07-21）：设置是整页路由、无 main.main-surface。
       CSS 靠这个属性区分「应用页 / 设置页」，比 :has() 可靠得多
       —— 实测 :has() 判据曾误伤（应用页 DOM 里也留着 div.main-surface）。 */
    setAttribute(root, "data-ds-route",
      document.querySelector("main.main-surface") ? "app" : "settings");
  ...
  const syncRouteState = (shell, { layout = false } = {}) => {
    ...
    try { ensureDraggablePolaroid(Boolean(home)); } catch {}
  };

  /* ---- 可拖拽/缩放/旋转拍立得（Yihan 2026-07-21）----
     从 CSS 伪元素改成真实 DOM，用户可在客户端直接操作；
     位置/缩放/角度存 localStorage，跨会话保留。整段 try/catch 包住，
     失败也绝不影响皮肤主体。 */
  const POLA_ID = "ds-polaroid-draggable";
  const POLA_KEY = "__ds_polaroid_transform_v1__";
  let polaDragBound = false;
  const readPola = () => {
    try { return JSON.parse(localStorage.getItem(POLA_KEY) || "null"); } catch { return null; }
  };
  const savePola = (s) => { try { localStorage.setItem(POLA_KEY, JSON.stringify(s)); } catch {} };
  const applyPola = (el, s) => {
    el.style.left = s.x + "px";
    el.style.top = s.y + "px";
    el.style.transform = `rotate(${s.rot}deg) scale(${s.scale})`;
  };
  const ensureDraggablePolaroid = (isHome) => {
    let el = document.getElementById(POLA_ID);
    if (!isHome) { if (el) el.style.display = "none"; return; }
    if (!el) {
      el = document.createElement("div");
      el.id = POLA_ID;
      el.setAttribute("title", "拖动移位 · 拖右下角把手缩放/旋转 · 双击复位");
      el.innerHTML =
        '<div class="ds-pola-photo"></div>' +
        '<div class="ds-pola-cap">SCV-70 WHITE BASE</div>' +
        '<div class="ds-pola-handle" aria-hidden="true"></div>';
      document.body.appendChild(el);
      const saved = readPola();
      const def = {
        x: Math.max(12, window.innerWidth - 150),
        y: Math.max(12, window.innerHeight - 320),
        scale: 1, rot: 7,
      };
      const st = saved && Number.isFinite(saved.x) ? saved : def;
      applyPola(el, st);
      if (!polaDragBound) { bindPolaDrag(el); polaDragBound = true; }
    }
    el.style.display = "";
  };
  const bindPolaDrag = (el) => {
    const handle = el.querySelector(".ds-pola-handle");
    let mode = null, sx = 0, sy = 0, sl = 0, st = 0, sScale = 1, sRot = 7, sDist = 1, sAng = 0;
    const center = () => { const r = el.getBoundingClientRect(); return { cx: r.left + r.width / 2, cy: r.top + r.height / 2 }; };
    const cur = () => {
      const m = /rotate\(([-\d.]+)deg\)\s*scale\(([-\d.]+)\)/.exec(el.style.transform || "");
      return { rot: m ? parseFloat(m[1]) : 7, scale: m ? parseFloat(m[2]) : 1 };
    };
    const down = (e, m) => {
      mode = m; e.preventDefault(); e.stopPropagation();
      sx = e.clientX; sy = e.clientY;
      sl = parseFloat(el.style.left) || 0; st = parseFloat(el.style.top) || 0;
      const c = cur(); sScale = c.scale; sRot = c.rot;
      const ctr = center();
      sDist = Math.hypot(e.clientX - ctr.cx, e.clientY - ctr.cy) || 1;
      sAng = Math.atan2(e.clientY - ctr.cy, e.clientX - ctr.cx);
    ...
    const move = (e) => {
      if (mode === "move") {
        applyPola(el, { x: sl + (e.clientX - sx), y: st + (e.clientY - sy), scale: sScale, rot: sRot });
      } else if (mode === "sizerot") {
        const ctr = center();
        const dist = Math.hypot(e.clientX - ctr.cx, e.clientY - ctr.cy) || 1;
        const ang = Math.atan2(e.clientY - ctr.cy, e.clientX - ctr.cx);
        const scale = Math.min(3, Math.max(0.4, sScale * (dist / sDist)));
        const rot = sRot + (ang - sAng) * 180 / Math.PI;
    ...
    const up = () => {
      ...
      mode = null;
      const c = cur();
      savePola({ x: parseFloat(el.style.left) || 0, y: parseFloat(el.style.top) || 0, scale: c.scale, rot: c.rot });
    };
    el.addEventListener("pointerdown", (e) => down(e, "move"), true);
    handle.addEventListener("pointerdown", (e) => down(e, "sizerot"), true);
    el.addEventListener("dblclick", (e) => {
      e.preventDefault();
      const def = { x: Math.max(12, window.innerWidth - 150), y: Math.max(12, window.innerHeight - 320), scale: 1, rot: 7 };
      el.style.transition = "left .2s, top .2s, transform .2s";
      applyPola(el, def); savePola(def);
    });
  };
  ...
  const cleanup = () => {
    ...
    document.documentElement?.removeAttribute("data-ds-route");
